...
Agents run as non-root (UID 1000) with `no-new-privileges`, 1GB memory limit (includes Chrome + KasmVNC), 1 CPU quota, 256MB shared memory, and no host filesystem access. Secure for most use cases.
