...
When an agent needs a credential at runtime, it receives an opaque handle (`$CRED{name}`) instead of the raw secret. The handle is resolved by the mesh proxy at call time — the actual key never enters the agent container.
...
- **Security**: `no-new-privileges`, runs as non-root `agent` user (UID 1000), PID limit 256, read-only root filesystem, `cap_drop: ALL`, 100MB noexec tmpfs
- **Ports**: 8400 (FastAPI)
...
Multi-project namespaces allow you to run separate agent fleets with isolated configuration. Each project gets its own `agents.yaml`, `permissions.json`, and auto-scoped blackboard — keys are automatically namespaced per project so fleets don't collide.
