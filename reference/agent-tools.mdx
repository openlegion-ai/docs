...
Every agent has access to 41 built-in tools. Agents can also [create custom skills](/features/agents#self-extending-skills) at runtime and use [MCP tool servers](/features/mcp).
...
All agents share a persistent Camoufox (stealth Firefox) browser service running in a separate container. KasmVNC serves a live browser view on port 6080. Patchright connects via CDP for programmatic control.
