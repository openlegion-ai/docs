...
Every agent has access to 44 built-in tools. Agents can also [create custom skills](/features/agents#self-extending-skills) at runtime and use [MCP tool servers](/features/mcp).
...
All agents use a persistent Chrome + KasmVNC browser architecture. A Chromium instance runs with a persistent profile at `/data/browser_profile`, and KasmVNC serves a live browser view on port 6080. Patchright (stealth Playwright fork) connects via CDP for programmatic control.
